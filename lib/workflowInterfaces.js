...
const collectInterfaces = (root) => {
    ...
    for (const d of fs.readdirSync(stepsDir).sort()) {
        ...
        // Read-and-catch, not exists-then-read (CodeQL js/file-system-race): a directory entry
        // without an action.yml is simply not a composite action.
        let text;
        try {
        ...
        steps[d] = extractActionInterface(text, `steps/${d}/action.yml`);
    }
