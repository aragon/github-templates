...
const createRepository = () => {
    const directory = fs.mkdtempSync(path.join(os.tmpdir(), 'release-summary-'));
...
test('monorepo: keeps release-window commits and excludes everything the package tag ships', () => {
    ...
    try {
        commitFile(repository, 'apps/app/base.txt', 'base', 'feat: initial app');
        ...
        commitFile(repository, 'pnpm-lock.yaml', 'lockfile', 'chore: update shared lockfile');
        commitFile(
            repository,
            'apps/app/feature.ts',
            'feature',
        ...
        commitFile(repository, 'apps/app/part-a.ts', 'a', 'feat: internal branch commit A');
        commitFile(repository, 'apps/app/part-b.ts', 'b', 'feat: internal branch commit B');
        git(repository, ['checkout', 'main']);
        ...
        const packageTag = detectLatestTag('@aragon/app@*', repositoryGit);
        const commits = collectScopedCommits({
            baseRef: packageTag,
            patterns: ['apps/app/**', 'packages/assistant-chat/**', 'pnpm-lock.yaml'],
            git: repositoryGit,
        });
...
test('single-package: v* boundary, no path filter, drops both release-commit styles', () => {
    const repository = createRepository();

    try {
        commitFile(repository, 'src/a.ts', 'a', 'feat: shipped in v1.0.0');
        commitFile(repository, 'package.json', '{"version":"1.0.0"}', 'chore(release): v1.0.0');
        git(repository, ['tag', 'v1.0.0']);
        commitFile(repository, 'src/b.ts', 'b', 'fix: after the release (#12)');
        commitFile(repository, 'package.json', '{"version":"1.0.1"}', 'Release v1.0.1');
        commitFile(repository, 'src/c.ts', 'c', 'feat: newest change');

        const repositoryGit = (args) => runGit(args, { cwd: repository });
        const tag = detectLatestTag('v*', repositoryGit);
        const commits = collectScopedCommits({ baseRef: tag, git: repositoryGit });
...
test('treats a workspace without package tags as a first release', () => {
    ...
    try {
        commitFile(repository, 'apps/assistant/index.ts', 'assistant', 'feat: unrelated app');
        commitFile(repository, 'apps/new-app/index.ts', 'new app', 'feat: first new-app feature');

        const repositoryGit = (args) => runGit(args, { cwd: repository });
        const packageTag = detectLatestTag('@aragon/new-app@*', repositoryGit);
        const commits = collectScopedCommits({
            baseRef: '',
...
const runGenerateSummary = async (repository, issuesById, { linearToken = 'lin_api_test' } = {}) => {
    ...
    try {
        ...
        delete process.env.PATH_PATTERNS;
        delete process.env.RELEASE_COMMIT_PATTERN;
...
test('warns about open Linear tickets and their current status', async () => {
    ...
    try {
        commitFile(repository, 'src/open.ts', 'open', 'feat(APP-100): open work (#1)');
        commitFile(repository, 'src/done.ts', 'done', 'fix(APP-200): finished work (#2)');
        commitFile(repository, 'src/phantom.ts', 'phantom', 'chore(APP-300): unresolvable id (#3)');
        ...
        // Finished and unresolvable tickets stay out of the warning section.
        assert.doesNotMatch(warningSection, /APP-200|APP-300/);
...
test('sends Linear personal keys bare and everything else as Bearer', () => {
    assert.equal(linearAuthorization('lin_api_abc123'), 'lin_api_abc123');
    assert.equal(linearAuthorization('lin_oauth_xyz789'), 'Bearer lin_oauth_xyz789');
    assert.equal(linearAuthorization('Bearer already-prefixed'), 'Bearer already-prefixed');
});
...
test('reads named path filters from the central mapper', () => {
    ...
    try {
        ...
        assert.deepEqual(readPathFilter(filterPath, 'app'), ['apps/app/**', 'pnpm-lock.yaml']);
        assert.throws(() => readPathFilter(filterPath, 'assistant'), /missing or invalid/);
    } finally {
